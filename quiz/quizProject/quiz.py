...
def quiz_tests(no):
    ...
    if request.POST:
        ...
        return template('quiz_result', resultans=resultans, resultquest=resultquest, success=success, realans=realans, topic=topic)

    else:

        questions = random.sample(range(0, len(result)-1), numQ)
        print("numQ= " + str(numQ))
